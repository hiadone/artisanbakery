...
class Board extends CI_Controller
{
	...
	public function latest_main($config)
	{
		...
		if ($cache_minute> 0) {
			...
			$cache_exclude_brd_id = is_array($exclude_brd_id) ? implode('-', $exclude_brd_id) : $exclude_brd_id;
			$cache_exclude_brd_key = is_array($exclude_brd_key) ? implode('-', $exclude_brd_key) : $exclude_brd_key;
			$cachename = 'latest/latest_main-s-' . $skin . '-i-' . $cache_brd_id . '-k-' . $cache_brd_key . '-l-' . $cache_exclude_brd_id . '-k-' . $cache_exclude_brd_key . '-l-t-' . $length . '-g-' . $is_gallery . '-w-' . $image_width . '-h-' . $image_height . '-p-' . $period_second;
			$html = $this->CI->cache->get($cachename);
			if ($html) {
				return $html;
			}
		...
		$result = $this->CI->db->get();
		$view['view']['latest_main'] = $latest_main = $result->result_array();

		
		if ($latest_main && is_array($latest_main)) {
			foreach ($latest_main as $key => $value) {
				$view['view']['latest_main'][$key]['name'] = display_username(
					element('post_userid', $value),
					element('post_nickname', $value)
				);
				$brd_key = $this->CI->board->item_id('brd_key', element('brd_id', $value));
				$view['view']['latest_main'][$key]['url'] = post_url($brd_key, element('post_id', $value));
				$view['view']['latest_main'][$key]['title'] = $length ? cut_str(element('post_title', $value), $length) : element('post_title', $value);
				$view['view']['latest_main'][$key]['display_datetime'] = display_datetime(element('post_datetime', $value), '');
				$view['view']['latest_main'][$key]['category'] = '';
				if (element('post_category', $value)) {
						$view['view']['latest_main'][$key]['category'] = $this->CI->Board_category_model->get_category_info(element('brd_id', $value), element('post_category', $value));
				}
				if ($is_gallery) {
					if (element('post_image', $value)) {
						$imagewhere = array(
		...
		$view['view']['skinurl'] = base_url( VIEW_DIR . 'latest/' . $skin);
		$html = $this->CI->load->view('latest/' . $skin . '/latest_main', $view, true);
	 ...
	 * 최근게시물을 가져옵니다
	 */
	public function latest_main_free($config)
	{
		$view = array();
		$view['view'] = array();
		...
		$view['view']['skinurl'] = base_url( VIEW_DIR . 'latest/' . $skin);
		$html = $this->CI->load->view('latest/' . $skin . '/latest_main_free', $view, true);
